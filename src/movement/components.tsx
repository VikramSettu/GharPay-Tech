...
export function DraftChip({ code }: { code: DraftCode | null }) {
  ...
  return (
    <span className={cn("px-1.5 py-0.5 rounded text-[10px] font-semibold", tone)}>{code}</span>
  );
}
...
export function DraftingPanel({ list, meta }: { list: MovementState[]; meta: Meta }) {
  ...
  const batchRows = useMemo(
    () =>
      batch
        ? (batch.ulids
            .map((u) => list.find((s) => s.ulid) === u))
            .filter(Boolean) as MovementState[]
        : [],
    [batch, list],
  );
    ...
    <div className="space-y-3">
      ...
      {cur ? (
        <div className="rounded-lg border border-border bg-card p-4">
          ...
          <div className="text-xs text-muted-foreground">
            {meta.get(cur.ulid)?.phone} · {meta.get(cur.ulid)?.area} · last message{" "}
            {rel(cur.lastCustomerMsgAt)} ago
          </div>
      ...
      <div className="rounded-lg border border-border bg-card divide-y divide-border">
        {(batch ? batchRows : queue).map((s, idx) => (
          <button
            key={s.ulid}
            onClick={() => mv.toggleSelect(s.ulid)}
          ...
          >
            ...
            <span className="text-xs font-medium truncate flex-1">
              {meta.get(s.ulid)?.name ?? s.ulid}
            </span>
            <span className="text-[10px] text-muted-foreground">
              {rel(s.lastCustomerMsgAt)} ago
            </span>
...
export function ActiveList({
  ...
  const [viewModalUlid, setViewModalUlid] = useState<string | null>(null);
  const [workModalUlid, setWorkModalUlid] = useState<string | null>(null);
  const [journeyModalUlid, setJourneyModalUlid] = useState<string | null>(null);
  ...
  return (
    <div className="space-y-3">
      <div className="rounded-lg border border-border bg-card overflow-hidden">
        <div className="grid grid-cols-[40px_1fr_100px_200px] px-3 py-2 border-b border-border bg-muted/50 text-[11px] uppercase tracking-wider text-muted-foreground font-semibold">
        ...
        <div className="divide-y divide-border">
          {paginatedRows.map((r, idx) => (
            <ActiveRow
              key={r.ulid}
              n={page * pageSize + idx + 1}
              r={r}
              meta={meta}
              active={selected === r.ulid}
              onWork={() => setWorkModalUlid(r.ulid)}
              onView={() => setViewModalUlid(r.ulid)}
              onJourney={() => setJourneyModalUlid(r.ulid)}
            />
          ))}
          {!rows.length && (
        ...
        <div className="px-3 py-2 border-t border-border bg-muted/30 flex items-center justify-between">
          <span className="text-[10px] text-muted-foreground">
            Page {page + 1} of {Math.ceil(rows.length / pageSize)} ({rows.length} total)
          </span>
      ...
      <Dialog open={!!viewModalUlid} onOpenChange={(open) => !open && setViewModalUlid(null)}>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div className="space-y-1">
                  ...
                  <div className="font-medium">{meta.get(viewModalUlid)?.name ?? viewModalUlid}</div>
                </div>
                <div className="space-y-1">
                  <div className="text-muted-foreground text-xs">Phone</div>
                  <div className="font-medium">{meta.get(viewModalUlid)?.phone ?? "—"}</div>
                </div>
                <div className="space-y-1">
                  <div className="text-muted-foreground text-xs">Area</div>
                  <div className="font-medium">{meta.get(viewModalUlid)?.area ?? "—"}</div>
                </div>
                <div className="space-y-1">
                  <div className="text-muted-foreground text-xs">ID</div>
                  <div className="font-mono text-xs">{viewModalUlid}</div>
                </div>
              </div>
              <div className="rounded-lg border border-border p-3 bg-muted/30 space-y-3">
                <div className="flex items-center justify-between">
                  <div className="text-xs font-semibold">Movement Status</div>
                  <Badge variant="outline" className="text-[10px]">
                    {states[viewModalUlid]?.stage ?? "—"}
                  </Badge>
                </div>
                <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-[11px]">
                  <div className="flex justify-between gap-2">
                    <span className="text-muted-foreground">Draft:</span>
                    <span className="font-medium"><DraftChip code={states[viewModalUlid]?.crmDraft} /></span>
                  </div>
                  <div className="flex justify-between gap-2">
                    <span className="text-muted-foreground">Work:</span>
                    <span className="font-medium">{states[viewModalUlid]?.work ?? "—"}</span>
                  </div>
                  <div className="flex justify-between gap-2">
                    <span className="text-muted-foreground">Waiting:</span>
                    <span className="font-medium">{states[viewModalUlid]?.customerWaitingSince ? rel(states[viewModalUlid].customerWaitingSince) + " ago" : "—"}</span>
                  </div>
                  <div className="flex justify-between gap-2">
                    <span className="text-muted-foreground">Next Action:</span>
                    <span className="font-medium">{states[viewModalUlid]?.nextAction?.kind ?? "—"}</span>
                  </div>
                </div>
              </div>
              <div className="flex justify-end gap-2 pt-2">
                <Button variant="ghost" size="sm" onClick={() => setViewModalUlid(null)}>
                  Close
                </Button>
                <Button 
                  size="sm" 
                  onClick={() => {
                    onSelect(viewModalUlid);
                    setViewModalUlid(null);
                  }}
                >
                  Start Work
                </Button>
      ...
      <Dialog open={!!workModalUlid} onOpenChange={(open) => !open && setWorkModalUlid(null)}>
        <DialogContent className="sm:max-w-[600px]">
          <WorkModalContent 
            ulid={workModalUlid!} 
            meta={meta} 
            onClose={() => setWorkModalUlid(null)} 
            onStartWork={() => {
              onSelect(workModalUlid!);
              setWorkModalUlid(null);
            }}
          />
        </DialogContent>
      </Dialog>

      <Dialog open={!!journeyModalUlid} onOpenChange={(open) => !open && setJourneyModalUlid(null)}>
        <DialogContent className="sm:max-w-[600px]">
          <JourneyTimeline ulid={journeyModalUlid!} />
          <div className="flex justify-end pt-4">
            <Button variant="ghost" size="sm" onClick={() => setJourneyModalUlid(null)}>Close</Button>
          </div>
        </DialogContent>
      </Dialog>
    </div>
...
function ActiveRow({
  ...
  return (
    <div
      className={cn(
        "grid grid-cols-[40px_1fr_100px_200px] items-center px-3 py-2.5 hover:bg-muted/50 transition",
        active && "bg-primary/5",
      )}
    >
      ...
      <div className="text-right">
        <Badge
          className={cn(
            "text-[10px]",
            r.bucket === "P0" ? "bg-destructive text-destructive-foreground" : "",
          )}
        >
          {r.bucket}
        </Badge>
...
export function WorkPanel({ ulid, meta }: { ulid: string | null; meta: Meta }) {
  ...
  const nextIn = (mins: number, kind: Parameters<typeof mv.setNextAction>[1]["kind"]) => {
    mv.setNextAction(ulid, {
      kind,
      dueAt: new Date(Date.now() + mins * 60000).toISOString(),
      ownerId: st.primaryOwnerId,
      ownerName: st.primaryOwnerName,
      note,
    });
  ...
  return (
    <div className="rounded-lg border border-border bg-card p-4 space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-2 text-[11px]">
        <Cell label="Draft">
          <DraftChip code={st.crmDraft} />
        </Cell>
        <Cell label="Stage">{st.stage}</Cell>
        <Cell label="Work">{st.work}</Cell>
        <Cell label="Next action">
          {st.nextAction
            ? `${st.nextAction.kind} · ${new Date(st.nextAction.dueAt).toLocaleTimeString()}`
            : "—"}
        </Cell>
        <Cell label="Waiting">
          {st.customerWaitingSince ? `${rel(st.customerWaitingSince)} ago` : "—"}
        </Cell>
        <Cell label="Tour">
          {st.tourAt
            ? `${new Date(st.tourAt).toLocaleString()}${st.tourConfirmed ? " ✓" : " (unconfirmed)"}`
            : "—"}
        </Cell>
      </div>
        ...
        <div className="flex flex-wrap gap-1.5">
          {(["connected", "no-answer", "busy", "wrong-number", "rejected"] as const).map((r) => (
            <Button
              ...
              className="h-7 text-[11px]"
              onClick={() => {
                mv.logCall(ulid, r, note);
                toast.success(`Call logged: ${r}`);
              }}
            >
      ...
      <div className="space-y-2">
        ...
        <div className="flex flex-wrap gap-1.5">
          <Button
            ...
            variant="outline"
            className="h-7 text-[11px]"
            onClick={() => mv.qualify(ulid, true, st.checkInDate)}
          >
            Definitely close
          </Button>
          <Button
            ...
            variant="outline"
            className="h-7 text-[11px]"
            onClick={() => mv.setStage(ulid, "matched", "Properties shared")}
          >
            Shared options
          </Button>
          <Button
            ...
            variant="outline"
            className="h-7 text-[11px]"
            onClick={() => mv.scheduleTour(ulid, new Date(Date.now() + 86400000).toISOString())}
          >
            Schedule tour
          </Button>
          <Button
          ...
          <Button
            ...
            variant="outline"
            className="h-7 text-[11px]"
            onClick={() => mv.tourOutcome(ulid, "positive")}
          >
            Positive
          </Button>
          <Button
          ...
          <Button
            ...
            variant="outline"
            className="h-7 text-[11px]"
            onClick={() => mv.prebook(ulid, "payment-intent")}
          >
            Payment intent
          </Button>
          <Button
          ...
          <Button
            ...
            variant="destructive"
            className="h-7 text-[11px]"
            onClick={() => mv.exit(ulid, "no-response", note)}
          >
            Exit
          </Button>
        </div>
      ...
      <div className="space-y-2">
        ...
        <Input
          ...
          placeholder="Note for the next step…"
        ...
        <div className="flex flex-wrap gap-1.5">
          ...
          <Button
            ...
            variant="outline"
            className="h-7 text-[11px]"
            onClick={() => nextIn(1440, "recheck-later")}
          >
            Tomorrow
          </Button>
          {st.nextAction && (
            <Button
              ...
              variant="ghost"
              className="h-7 text-[11px]"
              onClick={() => mv.completeNextAction(ulid)}
            >
            ...
            {f.map((row) => (
              <div key={row.stage} className="flex items-center gap-2">
                ...
                <span className="text-[11px] w-16 text-right tabular-nums">
                  {row.count}
                  {row.dropFromPrev > 0 && (
                    <span className="text-destructive"> −{row.dropFromPrev}</span>
                  )}
                </span>
              </div>
            ))}
          ...
          <div className="space-y-1">
            {lk.map((l) => (
              <div key={l.label} className="flex items-center justify-between text-xs py-1">
                ...
                <Badge variant={l.count > 0 ? "destructive" : "outline"} className="text-[10px]">
                  {l.count}
                </Badge>
              </div>
            ))}
      ...
      <div className="divide-y divide-border">
          ...
          <div key={u.id} className="px-3 py-2 flex items-center justify-between text-xs">
            <div>
              ...
              <div className="text-[11px] text-muted-foreground">
                {u.reason} · {u.waAccount} · {rel(u.ts)} ago
              </div>
            </div>
            <Button
              size="sm"
        ...
        {!unmatched.length && (
          <div className="p-6 text-center text-sm text-muted-foreground">
            Every conversation matched a CRM record.
          </div>
        )}
      </div>
    </div>
...
export function WorkModalContent({ 
  ulid, 
  meta, 
  onStartWork, 
  onClose 
  ...
  const mv = useMovement();
  const st = useMovement((s) => s.states[ulid]);
  const info = meta.get(ulid);
  ...
  return (
    <div className="space-y-6 p-1">
      <div className="flex items-center justify-between">
        <div className="space-y-1">
          <div className="text-xl font-bold">{info?.name ?? ulid}</div>
          <div className="text-xs text-muted-foreground">
            {info?.phone} · {info?.area} · owner {st.primaryOwnerName}
          </div>
        </div>
        <Button size="sm" className="bg-orange-500 hover:bg-orange-600 text-white" onClick={onStartWork}>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="rounded-lg border border-border p-3 bg-card space-y-1">
          <div className="text-[10px] uppercase text-muted-foreground font-semibold">Draft</div>
          <DraftChip code={st.crmDraft} />
        </div>
        <div className="rounded-lg border border-border p-3 bg-card space-y-1">
          <div className="text-[10px] uppercase text-muted-foreground font-semibold">Stage</div>
          <div className="text-sm font-medium">{st.stage}</div>
        </div>
        <div className="rounded-lg border border-border p-3 bg-card space-y-1">
          <div className="text-[10px] uppercase text-muted-foreground font-semibold">Work</div>
          <div className="text-sm font-medium">{st.work}</div>
        </div>
        <div className="rounded-lg border border-border p-3 bg-card space-y-1">
          <div className="text-[10px] uppercase text-muted-foreground font-semibold">Next Action</div>
          <div className="text-sm font-medium">{st.nextAction?.kind ?? "—"}</div>
        </div>
        <div className="rounded-lg border border-border p-3 bg-card space-y-1">
          <div className="text-[10px] uppercase text-muted-foreground font-semibold">Waiting</div>
          <div className="text-sm font-medium">{st.customerWaitingSince ? rel(st.customerWaitingSince) + " ago" : "—"}</div>
        </div>
        <div className="rounded-lg border border-border p-3 bg-card space-y-1">
          <div className="text-[10px] uppercase text-muted-foreground font-semibold">Tour</div>
          <div className="text-sm font-medium">{st.tourAt ? rel(st.tourAt) + " ago" : "—"}</div>
        </div>
      </div>

      <div className="space-y-3">
        <div className="text-[11px] uppercase text-muted-foreground font-semibold tracking-wider">Call</div>
        <div className="flex flex-wrap gap-2">
          {(["connected", "no-answer", "busy", "wrong-number", "rejected"] as const).map((r) => (
            <Button key={r} size="sm" variant="outline" className="h-8 text-[11px] px-3">
      ...
      <div className="space-y-3">
        <div className="text-[11px] uppercase text-muted-foreground font-semibold tracking-wider">Movement</div>
        <div className="flex flex-wrap gap-2">
          {["Definitely close", "Shared options", "Schedule tour", "Confirm tour", "Tour done", "Positive", "Send quote", "Payment intent", "Booked", "Exit"].map((label) => (
            <Button key={label} size="sm" variant="outline" className="h-8 text-[11px] px-3">
              {label}
            </Button>
          ))}
        </div>
      ...
      <div className="space-y-3">
        <div className="text-[11px] uppercase text-muted-foreground font-semibold tracking-wider">
          Next Action (Mandatory before you leave)
        </div>
        <Input placeholder="Note for the next step..." className="h-10" />
        <div className="flex flex-wrap gap-2">
          {["Call in 30m", "WhatsApp in 2h", "Tomorrow"].map((label) => (
            <Button key={label} size="sm" variant="outline" className="h-8 text-[11px] px-3">
              {label}
            </Button>
          ))}
