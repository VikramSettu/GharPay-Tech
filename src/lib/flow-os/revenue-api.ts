 ...
 */
export function parseStructuredChatRows(
  text: string,
  screenshotKey = "pasted-1",
): StructuredChatRowInput[] {
  const lines = text
    .split(/\r?\n/)
    .map((line) => line.trim())
    .filter(Boolean)
  ...
  return lines.map((line, index) => {
    ...
    const seenState: SeenState = ["seen", "unseen", "unknown"].includes(seen.toLowerCase())
      ...
      : "unknown";
    const previewDirection: PreviewDirection = ["incoming", "outgoing", "unknown"].includes(
...
async function findExistingLead(phoneE164: string): Promise<FlowLeadRow | null> {
  ...
  const { data } = await db.from("leads").select("*").eq("phone", phoneE164).maybeSingle();
  ...
  return (
    ((candidates ?? []) as FlowLeadRow[]).find(
...
function leadUpdateFromObservation(
  row: StructuredChatRowInput,
  stageSuggestion: ReturnType<typeof classifyLastMessage>,
  operator?: FlowOperator,
) {
  const unreadCount = row.unreadCount ?? 0;
  const seenState = deriveSeenState({
    unreadCount,
    explicit: row.seenState,
...
export async function ingestScreenshotBatch(
  input: ScreenshotBatchInput,
  ...
  for (const sourceRow of input.rows) {
    ...
    const movement = movementSummary(
      previous
        ? {
            lastMessage: previous.last_message,
            unreadCount: previous.unread_count,
            seenState: previous.seen_state,
          }
        : undefined,
      {
        lastMessage: sourceRow.lastMessage,
        unreadCount: sourceRow.unreadCount,
        seenState: sourceRow.seenState,
      },
    );
    ...
    if (resolution === "non_customer") {
      ...
      if (lead) {
        const isExactDuplicate =
          ...
          (previous.visible_timestamp_raw ?? "").trim() ===
            (sourceRow.visibleTimestamp ?? "").trim() &&
        ...
        await db
          .from("lead_cycles")
          .insert({ lead_id: lead.id, cycle_no: 1, open_reason: "screenshot_intake" });
      }
    }
    ...
    if (lead) {
      ...
      if (!lead.current_mission || lead.current_mission === "Contact customer")
        patch.current_mission = stageSuggestion.suggestedMission;
    ...
    const { error: observationError } = await db.from("flow_screenshot_observations").insert({
      ...
      handler_id:
        sourceRow.handlerName && operator?.name === sourceRow.handlerName ? operator.id : null,
...
export async function resolveObservationIdentity(
  observationId: string,
  leadId: string,
  resolution: "matched_existing" | "new_lead" = "matched_existing",
) {
...
function roiScore(lead: FlowLeadRow, latestObservation: any, nextAction: any) {
  ...
  if (stage === "FUTURE" && (!nextAction?.due_at || safeDate(nextAction.due_at) > now))
    score -= 100;
...
export async function createDraft30() {
  ...
  const [{ data: leads }, { data: claims }, { data: actions }, { data: observations }] =
    await Promise.all([
  ...
  for (const action of actions ?? [])
    if (!actionByLead.has(action.lead_id)) actionByLead.set(action.lead_id, action);
  const observationByLead = new Map<string, any>();
  for (const obs of observations ?? [])
    if (!observationByLead.has(obs.lead_id)) observationByLead.set(obs.lead_id, obs);
  ...
  const ranked = ((leads ?? []) as FlowLeadRow[])
    .filter((lead) => {
      ...
      if (claim && claim.operator_id !== operator.id && safeDate(claim.expires_at) > Date.now())
        return false;
      return true;
    })
    .map((lead) => ({
      lead,
      ...roiScore(lead, observationByLead.get(lead.id), actionByLead.get(lead.id)),
    }))
  ...
  for (const candidate of ranked) {
    ...
    const suggestion = classifyLastMessage(
      observation?.last_message ?? candidate.lead.last_wa_message,
    );
...
export async function loadOperatorWork(): Promise<OperatorWorkState> {
  ...
  const leadById = new Map<string, FlowLeadRow>(
    (leads ?? []).map((lead: FlowLeadRow) => [lead.id, lead]),
  );
  const claimByLead = new Map<string, FlowWorkClaim>(
    (claims ?? []).map((claim: FlowWorkClaim) => [claim.lead_id, claim]),
  );
  const actionByLead = new Map<string, any>();
  for (const action of actions ?? [])
    if (!actionByLead.has(action.lead_id)) actionByLead.set(action.lead_id, action);
  ...
  return {
    ...
    activeTrayCount: views.filter(
      (v) => v.activeTray && !["done", "released", "passed", "future"].includes(v.state),
    ).length,
...
export async function moveLeadToFuture(input: {
  ...
  if (input.itemId)
    await db
      .from("flow_draft_items")
      .update({ state: "future", disposition: input.reason })
      .eq("id", input.itemId);
  ...
  const { data: claim } = await db
    ...
    .maybeSingle();
  if (claim?.id)
    await db.rpc("flow_release_claim", {
      p_claim_id: claim.id,
      p_operator_id: operator.id,
      p_reason: "future",
    });
...
export async function completeAndNext(input: {
  ...
  if (
    !input.nextActionAt &&
    !["BOOKED", "CHECKED_IN", "LOST"].includes((input.stage ?? "").toUpperCase())
  ) {
  ...
  await db
    .from("flow_draft_items")
    .update({ state: "done", disposition: input.disposition, completed_at: isoNow() })
    .eq("id", input.itemId);
  const { data: claim } = await db
    .from("flow_work_claims")
    .select("id")
...
export async function loadLabelColourMappings() {
  const { data, error } = await db
    .from("flow_label_colour_mapping")
    .select("*")
    .eq("active", true)
    .order("colour_key");
  if (error) throw error;
...
export async function loadLeadsPaginated(
  page = 1,
  limit = 20,
  ...
  if (filters.q) {
    ...
    query = query.or(`name.ilike.%${filters.q}%,phone.ilike.%${filters.q}%`);
  }

  if (filters.stage && filters.stage !== "all") {
    query = query.eq("status", filters.stage); // Assuming 'status' maps to 'stage' based on schema.ts knowledge or common patterns
  }
...
export async function loadLeakageBuckets(): Promise<LeakageBucket[]> {
  ...
  for (const obs of obsRows)
    if (obs.lead_id && !latestObsByLead.has(obs.lead_id)) latestObsByLead.set(obs.lead_id, obs);

  const whatsappOnly = obsRows.filter((o: any) => !o.lead_id && o.resolution !== "non_customer");
  const unresolved = obsRows.filter((o: any) =>
    ["pending", "identity_review"].includes(o.resolution),
  );
  ...
  const freshInboundUntouched = leadRows.filter((lead) => {
    ...
    if (!obs) return false;
    const inbound =
      obs.preview_direction === "incoming" ||
      obs.seen_state === "unseen" ||
      Number(obs.unread_count ?? 0) > 0;
    return inbound && safeDate(obs.captured_at) > safeDate(lead.last_operator_action_at);
  });
  const qualifiedNoTour = leadRows.filter(
    (lead) =>
      ["DOSSIER", "MATCHED"].includes((lead.pipeline_stage ?? "").toUpperCase()) &&
      classifyLastMessage(lead.last_wa_message).suggestedStage === "MATCHED",
  );
  const tourNoPost = leadRows.filter(
    (lead) =>
      (lead.pipeline_stage ?? "").toUpperCase() === "TOUR_IN_PROGRESS" &&
      safeDate(lead.last_operator_action_at) < Date.now() - 4 * 60 * 60 * 1000,
  );
  const positiveNoQuote = leadRows.filter(
    (lead) =>
      (lead.pipeline_stage ?? "").toUpperCase() === "POST_VISIT" &&
      classifyLastMessage(lead.last_wa_message).suggestedStage === "POST_VISIT",
  );
  const paymentBookingGap = leadRows.filter(
    (lead) =>
      (lead.pipeline_stage ?? "").toUpperCase() === "NEGOTIATION" &&
      /payment|token|upi|qr/i.test(lead.last_wa_message ?? ""),
  );
  const checkInRisk = leadRows.filter(
    (lead) =>
      ["BOOKED", "CHECK_IN_READY"].includes((lead.pipeline_stage ?? "").toUpperCase()) &&
      lead.movein_date &&
      safeDate(lead.movein_date) <= Date.now() + 24 * 60 * 60 * 1000,
  );
  const incompleteBatches = (batches ?? []).filter((b: any) => b.status !== "balanced");

  return [
    {
      key: "incomplete_batches",
      label: "Screenshot batches not fully reconciled",
      count: incompleteBatches.length,
      severity: "red",
      rows: incompleteBatches,
    },
    {
      key: "whatsapp_only",
      label: "Visible in WhatsApp, not linked to CRM",
      count: whatsappOnly.length,
      severity: "red",
      rows: whatsappOnly,
    },
    {
      key: "identity_review",
      label: "OCR / identity review",
    ...
    {
      key: "fresh_inbound",
      label: "New inbound not acted on",
      count: freshInboundUntouched.length,
      severity: "red",
      rows: freshInboundUntouched,
    },
    {
      key: "no_next_action",
      label: "No claim and no dated next action",
      count: noNextAction.length,
      severity: "red",
      rows: noNextAction,
    },
    {
      key: "qualified_no_tour",
      label: "Qualified / tour-ready with no tour movement",
      count: qualifiedNoTour.length,
      severity: "amber",
      rows: qualifiedNoTour,
    },
    {
      key: "tour_no_post",
      label: "Tour movement without post-tour closure",
      count: tourNoPost.length,
      severity: "red",
      rows: tourNoPost,
    },
    {
      key: "positive_no_quote",
      label: "Positive post-tour / quote due",
      count: positiveNoQuote.length,
      severity: "red",
      rows: positiveNoQuote,
    },
    {
      key: "payment_booking_gap",
      label: "Payment intent without completed booking",
      count: paymentBookingGap.length,
      severity: "red",
      rows: paymentBookingGap,
    },
    {
      key: "checkin_risk",
      label: "Booked / check-in risk within 24h",
      count: checkInRisk.length,
      severity: "red",
      rows: checkInRisk,
    },
  ];
...
export async function loadThreeDayTruthSummary() {
  ...
  const [{ data: observations }, { data: batches }] = await Promise.all([
    db
      .from("flow_screenshot_observations")
      .select("lead_id,phone_e164,resolution,captured_at")
      .gte("captured_at", since3d)
      .limit(20000),
    db
      .from("flow_screenshot_batches")
  ...
  const uniqueKeys = new Set(
    (observations ?? []).map(
      (o: any) => o.lead_id ?? o.phone_e164 ?? `unresolved:${o.resolution}:${o.captured_at}`,
    ),
  );
  return {
    ...
    unresolved: (observations ?? []).filter((o: any) =>
      ["pending", "identity_review"].includes(o.resolution),
    ).length,
  };
}
